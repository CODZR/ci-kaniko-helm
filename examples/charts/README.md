...
iosp.io/release: { { .Release.Name | quote } }
{ {- end -} }
```
平台管理实例需要的实例ID。
...
iosp.io/version: { { .Chart.Version | quote } }
iosp.io/service: { { .Chart.Name | quote } }
iosp.io/metrics-port: { { .Values.deployment.managementPort | quote } }
{ {- end -} }
```
...
iosp.io/logs-parser: { { .Values.logs.parser | quote } }
{ {- end -} }
```
...
iosp.io/metrics-group: { { .Values.metrics.group | quote } }
iosp.io/metrics-path: { { .Values.metrics.path | quote } }
{ {- end -} }
```
